...
app.delete('/usuarios', (req, res) => {
    ...
    const deleteComentariosQuery = 'DELETE FROM comentarios WHERE UsuarioID = (SELECT _id FROM usuarios WHERE correo = ?)';
    ...
    db.query(deleteFavoritosQuery, [correo], (err, results) => {
        ...
        db.query(deleteComentariosQuery, [correo], (err, results) => {
            if (err) {
                console.error('Error al eliminar los comentarios:', err);
                return res.status(500).json({ error: 'Error al eliminar los comentarios del usuario' });
            }
